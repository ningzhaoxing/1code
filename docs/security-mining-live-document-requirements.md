...
1. 当前任务是漏洞挖掘任务，且任务 prompt 提供实时记录路径时，必须维护指定路径的漏洞挖掘记录文件。
...
6. 最终 Markdown 报告应以完整聊天/执行链路、工具调用摘要、审批/纠偏记录、证据文件和这份实时记录共同作为输入。

任务启动时，前端或后端需要把 resolved record path 注入给 Agent，例如：

```text
本次漏洞挖掘实时记录文件路径为：
<resolved-record-path>

请使用漏洞挖掘 skill，并在执行过程中持续维护该 Markdown 文件。
```
...
9. 最终报告入口在聊天末尾或工具栏出现，生成 `漏洞挖掘报告.md` 后可在右侧 Markdown 预览器打开和下载。
